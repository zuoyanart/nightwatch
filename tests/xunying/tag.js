...
module.exports = {
  ...
  before: function(client) {
    client
      .url(config.host + 'rmsearch')
      .waitForElementVisible('body', 1000);
  },
  '身份标签case直接点击搜索': function(client) {
    client.click("#btn-search");
    client.waitForElementPresent('.layui-layer', 1000);
  },
  '身份标签case搜索标签': function(client) {
    client.clearValue('#sfbq');
    client.setValue('#sfbq', "设计");
    client.click("#btn-search");
    client.pause(3000);
    client.assert.containsText(".list-wrap ul li:nth-child(1)", '设计');
  },
  '身份标签综合搜索': function(client) {
    client.clearValue('#sfbq');
    client.setValue("#sfbq", "设计");
    //点击所属行业
